...
export function drawBackground(ctx, s, now, metaRef) {
    ...
    ctx.lineWidth = 4; 
    ctx.beginPath(); ctx.moveTo(0, BATTLE_LINE_Y); ctx.lineTo(V_WIDTH, BATTLE_LINE_Y); ctx.stroke(); 
    ctx.setLineDash([]);
    ...
    ctx.fillRect(0, WALL_Y, V_WIDTH, V_HEIGHT - WALL_Y); 
    ...
    ctx.beginPath(); ctx.moveTo(0, WALL_Y); ctx.lineTo(gateL, WALL_Y); ctx.stroke(); 
    ctx.beginPath(); ctx.moveTo(gateR, WALL_Y); ctx.lineTo(V_WIDTH, WALL_Y); ctx.stroke();
    ...
    Object.entries(BARRACKS_DEFS).forEach(([key, def]) => {
        ...
        if (level === 0) {
            ...
            if (key === 'HATAMOTO') { 
                ...
                ctx.fillStyle = COLORS.ink; ctx.fillRect(-75, 0, 150, 30); 
                ctx.beginPath(); ctx.moveTo(-80, -75); ctx.lineTo(80, -75); ctx.lineTo(0, -130); ctx.fill(); 
            } else if (key === 'YUMI') { 
                ctx.fillStyle = COLORS.ink; ctx.beginPath(); ctx.moveTo(-80, -30); ctx.quadraticCurveTo(0, -80, 80, -30); ctx.lineTo(95, -20); ctx.lineTo(-95, -20); ctx.fill(); 
                ...
                ctx.fillStyle = COLORS.ink; ctx.beginPath(); ctx.moveTo(-95, -65); ctx.lineTo(95, -65); ctx.lineTo(75, -110); ctx.lineTo(-75, -110); ctx.fill(); 

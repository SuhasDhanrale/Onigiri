  ...
  if (s.gameState === 'CAMPAIGN_OVER') {
    return (
      <div className="fixed inset-0 bg-[var(--color-parchment)]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8">
        ...
        <p className="text-xl font-bold mb-8 text-[var(--color-ink-dark)]">The realm is unified.</p>
        <button
          onClick={() => { SoundManager.playSfx('ui_click'); setMeta(prev => ({ ...prev, honor: prev.honor + s.earnedHonor, conqueredRegions: [] })); initRun(); }}
          className="px-12 py-5 bg-[var(--color-ink-dark)] hover:bg-[var(--color-ink)] transition-colors text-[var(--color-parchment)] text-xl font-black uppercase border-4 border-[#d4af37]"
        >
          Begin Anew
        </button>
      </div>
  ...
  if (s.gameState === 'REGION_VICTORY') {
    ...
    return (
      <div className="absolute inset-0 bg-[var(--color-parchment)]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8 overflow-y-auto custom-scrollbar">
        ...
        <button
          onClick={() => { SoundManager.playSfx('ui_click'); handleRegionVictory(); }}
          className="px-12 py-5 bg-[#d4af37] text-[var(--color-ink-dark)] hover:bg-[#b5952f] transition-colors text-xl tracking-[0.3em] uppercase active:scale-95 font-black border-4 border-[var(--color-ink-dark)]"
        >
          Return to Map
        </button>
      </div>
  ...
  if (s.gameState === 'GAMEOVER') {
    return (
      <div className="absolute inset-0 bg-[var(--color-parchment)]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8 overflow-y-auto custom-scrollbar">
        ...
        <div className="bg-[var(--color-ink-dark)] text-[var(--color-parchment)] px-12 py-6 mb-8 text-center border-4 border-[#b84235]">
        ...
        <button
          onClick={() => { SoundManager.playSfx('ui_click'); setMeta(prev => ({ ...prev, honor: prev.honor + s.earnedHonor, conqueredRegions: [] })); initRun(); }}
          className="px-12 py-5 bg-[#b84235] hover:bg-[#a63b2f] transition-colors text-[var(--color-parchment)] text-xl tracking-[0.3em] uppercase active:scale-95 font-black border-4 border-[var(--color-ink-dark)]"
        >
          Enter the Cycle
        </button>
      </div>

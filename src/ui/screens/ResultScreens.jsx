...
export function ResultScreens({ s, setMeta, initRun, handleRegionVictory }) {
  if (s.gameState === 'CAMPAIGN_OVER') {
    return (
      <div className="fixed inset-0 bg-[#dfd4ba]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8">
        ...
        <button 
          ...
          className="px-12 py-5 bg-[#1b1918] text-[#dfd4ba] text-xl font-black uppercase border-4 border-[#d4af37]"
        >
          Begin Anew
        </button>
      </div>
    ...
    return (
      <div className="absolute inset-0 bg-[#dfd4ba]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8 overflow-y-auto custom-scrollbar">
        ...
        <button 
          ...
          className="px-12 py-5 bg-[#d4af37] text-[#1b1918] text-xl tracking-[0.3em] uppercase active:scale-95 font-black border-4 border-[#1b1918]"
        >
          Return to Map
        </button>
      </div>
  ...
  if (s.gameState === 'GAMEOVER') {
    return (
      <div className="absolute inset-0 bg-[#dfd4ba]/95 flex flex-col items-center justify-center z-50 pointer-events-auto p-8 overflow-y-auto custom-scrollbar">
        ...
        <button 
          onClick={() => { setMeta(prev => ({ ...prev, honor: prev.honor + s.earnedHonor, conqueredRegions: [] })); initRun(); }} 
          className="px-12 py-5 bg-[#b84235] text-[#dfd4ba] text-xl tracking-[0.3em] uppercase active:scale-95 font-black border-4 border-[#1b1918]"
        >
          Enter the Cycle
        </button>
      </div>

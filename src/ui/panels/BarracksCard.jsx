    ...
    >
      <div className="flex items-center h-14 px-2 relative">
        ...
        <div className="w-16 flex flex-col items-center justify-center shrink-0 relative z-10 border-r-2 border-[#1b1918] h-full bg-[#2b3d60] mr-2">
          ...
          {level > 0 && <div className="absolute -top-1 -left-1 bg-[#b84235] text-white text-[8px] font-bold px-1 py-0.5 border border-[#1b1918]">Lv.{s.troopLevel[bKey]}</div>}
        </div>
      ...
      {isFocused && (
        <div className="flex flex-col border-t-2 border-[#1b1918] bg-[#dfd4ba] text-[#1b1918] p-1.5 gap-1.5">
          {level === 0 ? (
            <button onClick={(e) => { e.stopPropagation(); buildBarracks(bKey, baseCost, maxTime); }} className={`w-full py-2 text-[10px] font-black transition-colors border-2 border-[#1b1918] ${s.koku >= baseCost && s.gameState === 'COMBAT' ? 'bg-[#1b1918] text-[#d4af37] hover:bg-[#2b3d60]' : 'bg-[#cfc4af] text-[#8b8574] cursor-not-allowed'}`}>BUILD ({baseCost} K)</button>
          ) : (
            <>
              {!isAuto && (
                <button onClick={(e) => { e.stopPropagation(); hireDrill(bKey, autoCost); }} className={`w-full py-1.5 text-[10px] font-black transition-colors border-2 border-[#1b1918] ${s.koku >= autoCost && s.gameState === 'COMBAT' ? 'bg-[#1b1918] text-[#d4af37] hover:bg-[#2b3d60]' : 'bg-[#cfc4af] text-[#8b8574] cursor-not-allowed'}`}>HIRE DRILL ({autoCost} K)</button>
              )}
              <div className="flex gap-1.5">
                <button onClick={(e) => { e.stopPropagation(); upgradeTroopLevel(bKey, costLvl); }} className={`flex-1 py-1.5 flex items-center justify-center transition-colors border-2 border-[#1b1918] ${s.koku >= costLvl && s.gameState === 'COMBAT' ? 'bg-[#1b1918] text-[#dfd4ba] hover:bg-[#d4af37] hover:text-[#1b1918]' : 'bg-[#cfc4af] text-[#8b8574] cursor-not-allowed'}`}>
                  <span className="text-[8px] font-black tracking-tighter leading-tight text-center">UPG DMG<br/>{costLvl} K</span>
                </button>
                <button onClick={(e) => { e.stopPropagation(); upgradeBarracksCap(bKey, costCap); }} className={`flex-1 py-1.5 flex items-center justify-center transition-colors border-2 border-[#1b1918] ${s.koku >= costCap && s.gameState === 'COMBAT' ? 'bg-[#1b1918] text-[#dfd4ba] hover:bg-[#d4af37] hover:text-[#1b1918]' : 'bg-[#cfc4af] text-[#8b8574] cursor-not-allowed'}`}>
              ...
              <div className="flex justify-between items-center mt-1 border-t-2 border-[#1b1918] pt-1.5 px-1">
                ...
                <div className="flex items-center gap-2 bg-[#1b1918] px-2 py-0.5 border border-[#4a5d23]">
                  <button onClick={(e) => { e.stopPropagation(); changeQuota(bKey, -1); }} className="hover:text-[#d4af37] text-lg leading-none cursor-pointer text-[#dfd4ba] px-1 font-bold">-</button>
                  <span className="text-[#dfd4ba] min-w-[2ch] text-center font-mono text-[10px] font-bold">{s.guardQuotas[bKey] || 0}</span>
                  <button onClick={(e) => { e.stopPropagation(); changeQuota(bKey, 1); }} className="hover:text-[#d4af37] text-lg leading-none cursor-pointer text-[#dfd4ba] px-1 font-bold">+</button>
                </div>
              </div>
            </>

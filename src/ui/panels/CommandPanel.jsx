...
export function CommandPanel({ 
  ...
  return (
    <div className={`flex-[3] min-w-[340px] max-w-[400px] h-full bg-[var(--color-parchment)] flex flex-col shrink-0 overflow-y-auto custom-scrollbar relative z-40 border-l-4 border-[var(--color-ink)] ${s.gameState === 'MAP_SCREEN' ? 'hidden' : ''}`}>
      
      <EconomyHeader state={s} activeTroops={activeTroops} maxTroops={maxTroops} />
      
      <div className="p-4 flex flex-col gap-3 bg-[var(--color-parchment)] border-b-4 border-[var(--color-ink)]">
        <h3 className="text-[var(--color-ink)] font-black text-[10px] uppercase tracking-[0.2em] border-b-2 border-[var(--color-ink)] pb-1">Military Forces</h3>

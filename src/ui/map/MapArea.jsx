...
export function MapArea({ meta, startCombat }) {
  return (
    <div className="flex-1 relative bg-[var(--color-parchment)] overflow-hidden">
      ...
      <div className="absolute top-12 left-1/2 -translate-x-1/2 text-center z-10 pointer-events-none">
        <h1 className="text-6xl font-black text-[var(--color-ink)] tracking-[0.5em] uppercase drop-shadow-md">War Map</h1>
        <p className="text-xl font-bold text-[#b84235] tracking-widest uppercase mt-4 bg-[var(--color-parchment)] inline-block px-4 py-1 border-2 border-[#b84235]">Select Your Conquest</p>
      </div>
      ...
      <div className="absolute z-20 flex flex-col items-center" style={{ left: '50%', top: '88%', transform: 'translate(-50%, -50%)' }}>
        <div className="relative w-24 h-24 rounded-full border-4 border-[#d4af37] bg-[#2b3d60] flex items-center justify-center shadow-[0_0_40px_rgba(212,175,55,0.4)]">
            ...
            <div className="absolute top-full mt-3 px-4 py-1.5 font-black text-xs uppercase tracking-widest whitespace-nowrap border-2 shadow-lg bg-[var(--color-ink)] text-[#d4af37] border-[#d4af37]">
                Imperial Capital
            </div>
            <div className="absolute -bottom-10 text-[10px] font-bold text-[var(--color-ink)] tracking-widest uppercase">Your Stronghold</div>
        </div>
      </div>
      ...
      {Object.values(CAMPAIGN_MAP).map(region => {
        ...
        return (
          <div key={region.id} className="absolute group z-20" style={{ left: cx, top: cy, transform: 'translate(-50%, -50%)' }}>
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-56 p-3 bg-[var(--color-ink)] text-[var(--color-parchment)] opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none border-2 border-[#8b8574] shadow-2xl flex flex-col items-center">
            ...
            >
               ...
               <div className={`absolute top-full mt-3 px-3 py-1 font-black text-[10px] uppercase tracking-widest whitespace-nowrap border-2 shadow-lg ${isConquered ? 'bg-[#d4af37] text-[var(--color-ink)] border-[var(--color-ink)]' : 'bg-[var(--color-parchment)] text-[var(--color-ink)] border-[#b84235]'}`}>
                   {region.name}
               </div>
            </button>
          </div>

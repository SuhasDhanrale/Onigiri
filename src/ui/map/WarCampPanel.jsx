  ...
  return (
    <div className="w-[340px] h-full bg-[#dfd4ba] border-r-4 border-[#1b1918] flex flex-col p-4 relative z-20 shadow-[10px_0_20px_rgba(0,0,0,0.3)]">
              ...
              return (
                <div key={key} className={`p-2 border-2 transition-colors ${isEquipped ? 'bg-[#b84235] border-[#1b1918] text-[#dfd4ba]' : 'bg-[#e8e0cc] border-[#8b8574] text-[#1b1918]'}`}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {isUnlocked ? (
                      <button onClick={() => equipHeirloom(key)} className={`px-2 py-0.5 text-[8px] font-bold uppercase border-2 transition-colors ${isEquipped ? 'bg-[#1b1918] border-[#1b1918] text-[#dfd4ba]' : 'bg-[#1b1918] border-[#1b1918] text-[#dfd4ba] hover:bg-[#dfd4ba] hover:text-[#1b1918]'}`}>
                        {isEquipped ? 'Equipped' : 'Equip'}
                      </button>
                    ) : (
                      <button onClick={() => unlockHeirloom(key, h.cost)} disabled={meta.honor < h.cost} className="px-2 py-0.5 text-[8px] font-bold uppercase border-2 border-[#1b1918] bg-[#d4af37] text-[#1b1918] disabled:opacity-50 disabled:grayscale hover:bg-[#b5952f] transition-colors">
                        Unlock ({h.cost} H)
                      </button>
                    )}
                  </div>
                ...
                <div key={key} className={`p-2 border-2 transition-colors ${isUnlocked ? 'bg-[#2b3d60] border-[#1b1918] text-[#dfd4ba]' : 'bg-[#e8e0cc] border-[#8b8574] text-[#1b1918]'}`}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {isUnlocked ? (
                      ...
                      <button onClick={() => unlockTech(key, t.cost)} disabled={meta.honor < t.cost} className="px-2 py-0.5 text-[8px] font-bold uppercase border-2 border-[#1b1918] bg-[#d4af37] text-[#1b1918] disabled:opacity-50 disabled:grayscale hover:bg-[#b5952f] transition-colors">
                        Unlock ({t.cost} H)
                      </button>
                    )}
                  </div>
      ...
      <button 
        ...
        className="mt-4 w-full py-3 bg-[#1b1918] text-[#dfd4ba] text-[10px] font-black tracking-[0.2em] uppercase border-4 border-[#b84235] hover:bg-[#b84235] hover:text-[#1b1918] transition-colors flex justify-center items-center gap-2 group"
      >
